        ...
        try:
            with mock.patch.object(package, "_progress_adapter", return_value=adapter), \
                ...
                for turn in ("initial-turn", "native-continuation-turn"):
                    ...
                    try:
                        # Agent initialization binds the durable id after gateway routing.
...
class LedgerTests(unittest.TestCase):
    ...
    def test_populated_v5_database_migrates_to_v8_without_losing_rows(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(
                ledger._db.execute("PRAGMA user_version").fetchone()[0], 11
            )
            self.assertEqual(
                ledger._db.execute(
    ...
    def test_activation_dispatch_ambiguity_is_restart_durable_and_not_replayable(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(recovered._db.execute("PRAGMA user_version").fetchone()[0], 11)
            recovered.close()
