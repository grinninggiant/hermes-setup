...
Non-streamed Linear AgentSession turns use only documented Hermes plugin/platform surfaces. This path is inert unless profile-local `native_goal_continuation_enabled: true` is set. The plugin records Hermes' structured `on_session_end` result and holds final Linear delivery until the adapter's `on_processing_complete` boundary, after core `gateway/run_goals.py` has judged the standing goal and enqueued any ordinary continuation. The adapter then re-reads the AgentSession, issue, delegate, workflow state, and every blocker page, combines that live evidence with the immutable structured turn result and local Stop/closure fences, and persists exactly one of `success`, `continue`, `awaiting_input`, `approval`, `blocked`, or `stopped`. Ambiguous or incomplete evidence is `blocked`; only checked acceptance plus a native `done` goal may deliver the model response as a final Linear `response`.

At `handle_message` ingress, before the base adapter schedules model execution, the adapter resolves the authoritative Linear binding and Hermes session and creates the initial bounded issue goal and `GoalContract` through `ensure_goal_for_source`. All goal reads, prompts, and exceptional resumes use the corresponding source-scoped public gateway operations, so profile selection remains core-owned. Hermes alone calls `evaluate_after_turn`, owns ordinary continuation FIFO admission, and owns session-scoped process/delegation waits. The plugin never pauses that ownership. There are only two plugin-scheduled exceptions: if the native judge says `done` while live Linear acceptance is still unchecked, the adapter durably records resume intent, resumes the same goal without resetting its budget, records the observed active phase, and admits exactly one continuation; if the native goal auto-paused specifically because `goals.max_turns` was exhausted, the adapter re-runs fresh Linear gates, durably consumes one bounded rollover, and calls the public resume operation with `reset_budget=True`. The default rollover cap is three and can be narrowed with profile-local `goal_max_budget_rollovers`.

A non-final turn summary is persisted through the existing outbox as an ephemeral `thought`, never as a normal response; native goal/loop status notices are suppressed by the typed platform policy instead of leaking as final responses. Exceptional plugin continuations use public `gateway.wake.admit_internal_event`, preserve the original `SessionSource`, pin the exact Hermes session, and require its concrete scheduling receipt. Because Hermes skips `pre_gateway_dispatch` for internal events and native goal wakes carry no Linear metadata, the adapter's `handle_message` resolves and stamps every internal event targeting a durably bound Linear AgentSession, then revalidates Stop/cancel/Done/input/approval/delegate/blocker/closure and H2-scoped acceptance gates before `BasePlatformAdapter` may schedule model execution. The public `allow_internal_execution` boundary repeats those authoritative gates immediately before the handler runs, closing the admission-to-execution race. Unbound internal events retain ordinary Hermes behavior. Intentional terminal vetoes are acknowledged and fenced, while transient read/admission failures remain recoverable. SQLite bounds admission to three durable attempts across restart; `running` rows are never replayed. A delayed successful response revalidates all lifecycle and acceptance gates again immediately before Linear dispatch.

Schema v9 adds `turn_decisions`: decision ID, Linear AgentSession UUID, issue ID, Hermes session ID, goal generation, ordinal, outcome, dispatch state, bounded error, timestamps, and the payload-free public source-routing snapshot needed for restart. Schema v10 adds the payload-free `goal_budget_rollovers` counter and in-flight rollover marker; `goal_resume_recovery` records only the explicit pre/post-resume phase for crash reconciliation. These tables store no prompt, summary, description, promptContext, credential, or issue-body content.
...
```yaml
# Acceptance candidate only; no profile configuration is changed by this patch
outbound_mcp:
