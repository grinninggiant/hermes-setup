...
- General uses its reviewed immutable gateway release for both the gateway and restricted maintenance-send launcher. The shell execution test compares their actual bootstrap argv, so a future general release change must keep both launchers aligned. The other eight send profiles retain their previous bootstrap arguments; this general-only correction does not repair or promote their runtime selection. Read current source and serving coordinates rather than treating older `deployment/general-runtime-pin.*.diff` files as the live release.
