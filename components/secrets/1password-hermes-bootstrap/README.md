...
- Source/LIVE drift for OPS221: the reviewed source wrapper selects the fixed `hermes-agent-b208667b53acff437e38a339c7d70d618ce7f90e/venv/bin/hermes` executable only for `general`; the existing LIVE wrapper already has the general profile pinned to `hermes-agent-293daf98684321b628c29adbd3eef064b1d3fdc9`, while the other eight profiles still pass `/Users/mutlupolatcan/.local/bin/hermes`. The exact review-only application and rollback diffs are `deployment/general-runtime-pin.apply.diff` and `deployment/general-runtime-pin.rollback.diff`; no plist reload or live mutation is part of this change.
