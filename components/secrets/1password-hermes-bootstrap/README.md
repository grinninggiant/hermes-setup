...
- Source/LIVE drift for OPS221: the reviewed source wrapper selects the fixed `hermes-agent-293daf98684321b628c29adbd3eef064b1d3fdc9/venv/bin/hermes` executable only for `general`; the current LIVE wrapper still passes `/Users/mutlupolatcan/.local/bin/hermes` for all nine profiles. The exact review-only application and rollback diffs are `deployment/general-runtime-pin.apply.diff` and `deployment/general-runtime-pin.rollback.diff`; no plist reload or live mutation is part of this change.
