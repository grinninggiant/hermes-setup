...
`~/.hermes/scripts/derya-gh` obtains the general profile's existing 1Password service-account bootstrap identity from Keychain, resolves only the four pinned GitHub App references through the official SDK, signs a short-lived App JWT in memory, verifies the pinned organization installation, all-repositories selection, and exact permission set, requests an organization-installation token, removes the bootstrap identity, and runs the pinned `/opt/homebrew/bin/gh` with a fresh mode-`0700` config directory. The wrapper never prints either credential and does not cache the installation token. Its command gate allows only exact `gh auth status`, the read-only installation-scope check, and canonical `repos/grinninggiant/<repository>` routes; other owners, aliases, extensions, token printing, custom hosts and generic gh commands fail closed. API mutations remain limited to the broker's explicit PR, merge, and comment operations; repository deletion is not exposed. Agent Git operations use the explicit `derya-gh-credential` helper over HTTPS and accept only canonical `grinninggiant/<repository>` paths. The host user's personal SSH identity remains available only outside Hermes agent sessions.
...
| --- | --- | --- | --- | --- | --- |
| Derya / `general` | `grinninggiant/*` | brokered HTTPS | brokered HTTPS | Repository admin + Contents/PR write; Actions read | active |
