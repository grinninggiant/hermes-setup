...
Derya's unattended GitHub API path uses the organization-owned `Derya Hermes` GitHub App. The App installation and broker boundary cover all current and future repositories owned by `grinninggiant`; accounts outside that exact organization remain fail-closed. The App ID, installation ID, organization scope, and private-key attachment live in the profile-scoped `Derya - Secrets` item. The private key is never mapped into the long-running gateway environment.

`~/.hermes/scripts/derya-gh` obtains the general profile's existing 1Password service-account bootstrap identity from Keychain, resolves only the four pinned GitHub App references through the official SDK, signs a short-lived App JWT in memory, verifies the pinned organization installation and all-repositories selection, reads the installation's current GitHub-granted permissions, requests only the token permissions for the validated operation, removes the bootstrap identity, and runs the pinned `/opt/homebrew/bin/gh` with a fresh mode-`0700` config directory. Extra App permissions no longer invalidate the broker; reads request read-level grants, allowed API writes require their specific grants, and the Git credential helper requests contents write plus workflows write when granted (Git does not distinguish fetch from push in credential requests). The wrapper never prints either credential and does not cache the installation token. Its command gate allows only exact `gh auth status`, the read-only installation-scope check, and canonical `repos/grinninggiant/<repository>` routes; other owners, aliases, extensions, token printing, custom hosts and generic gh commands fail closed. API mutations remain limited to the broker's explicit PR, merge, and comment operations; repository deletion is not exposed. Agent Git operations use the explicit `derya-gh-credential` helper over HTTPS and accept only canonical `grinninggiant/<repository>` paths. The host user's personal SSH identity remains available only outside Hermes agent sessions.
...
| --- | --- | --- | --- | --- | --- |
| Derya / `general` | `grinninggiant/*` | brokered HTTPS | brokered HTTPS | GitHub-granted read scopes; PR/merge/comment writes and Git contents/workflows writes only when required and granted | active |
