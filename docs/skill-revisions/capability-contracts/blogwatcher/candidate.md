...
By default the database lives at `~/.blogwatcher-cli/blogwatcher-cli.db`. With disposable `--rm` containers, data in the writable layer is lost when the container is removed unless persisted outside it. Use `BLOGWATCHER_DB` or a volume mount to persist it:
...
Before migrating from `Hyaxia/blogwatcher`, verify the installed version, source database, destination and schema compatibility. Preserve a verified backup and stop only the owned writer; never overwrite an existing destination with a blind move. Validate the migrated database through read-only CLI results before switching the configured path.
...
Resolve `BLOGWATCHER_DB` or `--db` to the authorized profile-owned database before modifying subscriptions or read state; do not silently use another profile's shared default. A scan changes local collection state even though it does not send notifications. After an authorized add/import/remove/read action, re-list the exact blog/article state. Distinguish fetch failures from an empty feed, and confirm deletion scope before using confirmation-suppression flags.
