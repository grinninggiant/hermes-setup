...
Do not disable sandboxing or approvals to work around a host failure. Report the exact denial and use an already-authorized execution surface for validation; do not present a narrow prompt or clean worktree as a replacement for security controls.

A read-only review sandbox may also deny pytest/git temporary files. Such a run proves zero tests, not a product failure. Use read-only reviewers for code inspection; run tests through the coordinator's normal authorized terminal or a workspace-write implementation task with isolated temporary HERMES_HOME. Preserve all existing edits, give parallel workers non-overlapping ownership, and inspect diffs plus real test output before accepting their reports.
