...
### 0. Keep race-test barriers outside contested locks

When reproducing an async-to-thread/store race, pause the worker before it acquires the store lock. Do not block inside an ownership callback under that lock while the event-loop thread synchronously resets the same store: that deadlocks the test rather than reproducing a stale write. Bound every threading wait, release barriers in `finally`, and cap each test-file subprocess so failures cannot orphan workers. Exercise the real caller plus async facade and locked store; calling the store directly will miss dropped keyword/ownership guards at the caller boundary.
