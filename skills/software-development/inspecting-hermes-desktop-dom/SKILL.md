...
CDP; inspect an authorized screenshot with the available vision tool for visual judgment, then ask the user only for preference or unresolved ambiguity.
...
For the source version using this module, dev-server runs default to loopback port 9222. The port remains closed for packaged builds, missing dev-server configuration, an explicit opt-out, or an invalid port. Check
...
`HERMES_DESKTOP_CDP_PORT` selects a valid port or disables debugging (`off`, `0`, `false`, `no`). The source accepts integer ports from 1024 through 65535. Validate this setting before substituting it into a URL; do not treat an opt-out value as a hostname port. Preserve the packaged-build gate.
...
An empty response is not proof that no listener exists: inspect curl status, connection/timeout errors and the configured endpoint. Do not silently guess another port.
...
- Verify build-mode claims against the running dev server and source version; a stale note is not runtime evidence.
