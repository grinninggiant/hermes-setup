...
metadata:
  hermes:
    ...
    related_skills: [notion, google-workspace, email-inbox-triage]
---

# Weekly Review and Planning

Run a bounded weekly reset across the user's chosen systems. A weekly review can be one-off or part of an existing schedule; the request alone does not establish that a scheduler, blueprint or recurring job exists. Create a recurring job only when explicitly requested, using the available scheduler's current schema and verifying its exact schedule and destination.
