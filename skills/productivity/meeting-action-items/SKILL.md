...
Use the user's authoritative task tracker and its governing workflow; do not infer that a note store is also the task system. Where Linear owns work and Notion owns durable knowledge, keep tasks/status in Linear and minutes in the approved Notion destination. Search for matching open items before creating anything—recurring meetings breed duplicates. Preserve owner/date/status conflicts for confirmation instead of silently overwriting. Distinguish proposed creates from updates.
