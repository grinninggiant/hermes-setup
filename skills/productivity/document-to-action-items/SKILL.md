...
Turn documents into cited facts and proposed actions. Extraction is not legal advice, and low-confidence OCR or ambiguous language must remain visible. The `pdf` / `docx` skills own extraction mechanics; this skill owns what happens to the extracted content.
...
Load `pdf` or `docx`. Extract text/tables while retaining file and page/section coordinates. For scans, record OCR confidence or visible quality issues. Done when every extracted field can cite its source location.
...
Use the authoritative task tracker for approved tasks, owners, status and acceptance criteria (Linear in this studio). Keep durable document synthesis in the approved knowledge store; load `notion-knowledge-ops` before any Notion write and resolve the canonical destination first. Calendar events or spreadsheets are separate requested outputs, not replacement task authorities. Attach document/page provenance and avoid copying unnecessary sensitive text. Read records back from the provider and verify owner/date/link. If a write times out ambiguously, search for the expected record before retrying. Done when every approved action is verified.
