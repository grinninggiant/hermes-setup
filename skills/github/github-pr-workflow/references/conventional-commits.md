...
Examples below illustrate syntax only; never copy their feature claims, issue numbers, co-author identities, URLs or version choices into a real commit. Follow repository conventions and describe only the actual change. Closure keywords can cause remote side effects: use them only for the verified intended GitHub issue with authorization. They do not authorize human-owned Linear Done. Use a non-closing reference by default. A footer is not test or deployment evidence.
